...
def discover_weekly_1(user: List[str], playlists: List[List[str]]) -> List[str]:
    ...
    """
    # recommended_playlists that have more then 3 songs a user listened to.
    recommended_playlists: List[List[str]] = []
    for playlist in playlists:
        start = 0
        end = len(playlist) - 1
        quicksort(playlist, start, end)
        # binum the number of user songs in a playlist generated by the binary search.
        binum = special_binary_search(playlist, user)
        # new_songs the maximum amount of songs a user could have listened to for a playlist to be recommended.
    ...
    # a randomly picked recommended playlist.
    recommended_playlists[i]
    discover_weekly: List[str] = []
    # for 5 iterations.
    for r in range(5):
        # picks a random song from the recommended playlist.
        song = random.choice(recommended_playlists[i])
        # keeps picking new songs till a song is found not yet in discover weekly's recommended list.
