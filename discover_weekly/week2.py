...
def discover_weekly_2(users: Dict[str, List[str]]) -> Dict:

    # initiating genres
    pop, rock, techno = get_genre()
    
    # looping through users and playlists
    for user, playlist in users.items():
        popc = 0
        rockc = 0
        technoc = 0
        ...
        if popc > rockc and popc > technoc:
            user_five_songs[user] = random.sample(pop, k=5)
        if rockc > popc and rockc > technoc:
            user_five_songs[user] = random.sample(rock, k=5)
        if technoc > popc and technoc > rockc:
            user_five_songs[user] = random.sample(techno, k=5)
    
    return user_five_songs
